...
pub async fn get_config_status(
    ...
    match AppType::from_str(&app).map_err(|e| e.to_string())? {
        ...
        AppType::Tuzi => {
            // Tuzi uses ~/.tuzi/ directory with JSON config like Claude
            let tuzi_dir = dirs::home_dir()
                .map(|h| h.join(".tuzi"))
                .unwrap_or_else(|| std::path::PathBuf::from(".tuzi"));
            let config_path = tuzi_dir.join("settings.json");
            let exists = config_path.exists();
            let path = tuzi_dir.to_string_lossy().to_string();
