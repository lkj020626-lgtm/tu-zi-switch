...
fn restore_official_at_paths_inner(paths: &ClaudeDesktopPaths) -> Result<(), AppError> {
    ...
    remove_tuzi_switch_enterprise_config(&paths.threep_config_path)?;
...
fn remove_tuzi_switch_enterprise_config(path: &Path) -> Result<(), AppError> {
    if !path.exists() {
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn claude_desktop_restore_switches_to_1p_and_removes_tuzi_switch_profile() {
        let temp = TempDir::new().expect("tempdir");
        let paths = test_paths(temp.path());
        let provider = direct_provider("direct");
        let db = test_db();
